...
const createReleasePr = async (project: string, baseBranch: string) => {
  ...
  // console.log(`gh pr create --title "release ${project}: ${version}" --body "${summary}" --base ${baseBranch} --head $(git rev-parse --abbrev-ref head) --dry-run`)
  // const resp =
  //  await $`gh pr create --title "release ${project}: ${version}" --body "${summary}" --base ${baseBranch} --head $(git rev-parse --abbrev-ref head) --dry-run`
  //     .text();
  // console.log(resp);
...
program
  ...
  .action(async (project) => {
    ...
    try {
      ...
      console.log(mainPr);
      // logger.info(` - main: ${mainPr.data.url}`);

      const prodPr = await createReleasePr(project, "prod");
      // logger.info(` - prod: ${prodPr.data.url}`);
    } catch (e) {
      logger.error(e);
    } finally {
